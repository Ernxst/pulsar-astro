...
To release a new version of `pulsar`, find the `Version Packages` PR, read it over, and merge it.
...
### Releasing `pulsar@next` (aka "prerelease mode")
...
While in prerelease mode, follow the normal release process to release `pulsar@next` instead of `pulsar@latest`. To release `pulsar@latest` instead, see [Releasing `pulsar@latest` while in prerelease mode](#releasing-pulsarlatest-while-in-prerelease-mode).
...
Exiting prerelease mode should happen once an experimental release is ready to go from `npm install pulsar@next` to `npm install pulsar`. Only a core contributor run these steps. These steps should be run before
...
### Releasing `pulsar@latest` while in prerelease mode

When in prerelease mode, the automatic PR release process will no longer release `pulsar@latest`, and will instead release `pulsar@next`. That means that releasing to `latest` becomes a manual process. To release latest manually while in prerelease mode:
